...
describe('formatMathDate', () => {
  ...
  function getExpectedDate(locale: string) {
  ...
  it('formats date with en-US locale', () => {
    expect(formatMathDate(date, 'en-US')).toBe(getExpectedDate('en-US'))
  })

  it('formats date with de-DE locale', () => {
    expect(formatMathDate(date, 'de-DE')).toBe(getExpectedDate('de-DE'))
  })

  it('formats date with ru-RU locale', () => {
    expect(formatMathDate(date, 'ru-RU')).toBe(getExpectedDate('ru-RU'))
  })
})
